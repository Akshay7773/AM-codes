...
const updateUser = catchAsync(async (req, res) => {
  ...
  const userId = req.user._id.valueOf();
  const user = await userService.updateUserById(userId, obj, req.locals);
  
  res.send(user);
});

const savePostToUser = catchAsync(async (req, res) => {
  const postId = req.body.postId;
  const userId = req.user._id.valueOf();
  const user = await userService.savePostToUser(userId, postId);
  res.send(user);
});
