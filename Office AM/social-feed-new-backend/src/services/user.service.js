 ...
 * @returns {Promise<User>}
 */
const updateUserById = async (userId, updateBody, updateImage) => {
  const obj = { a: "1" };
  Object.assign(obj, { a: "33" });
  ...
  Object.assign(user, updateBody, updateImage ? updateImage : { image: "" });
  await user.save();
  console.log(user);
  return user;
};

const savePostToUser = async (userId, postId) => {
  const user = await getUserById(userId);
  if (!postId) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Post ID is required");
  }
  const allSavedPosts = user.savedPosts.filter((post) => post == postId);
  if (allSavedPosts.length > 0) {
    const filteredSavedPosts = user.savedPosts.filter((post) => post != postId);
    Object.assign(user, { savedPosts: filteredSavedPosts });
    await user.save();
    return user;
  } else {
    const addedNewSavedPost = [...user.savedPosts, postId];
    Object.assign(user, { savedPosts: addedNewSavedPost });
    await user.save();
    return 1;
  }
};
